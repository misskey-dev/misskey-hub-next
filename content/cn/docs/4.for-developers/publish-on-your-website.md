...
在 Misskey v2023.11.0 及更高版本里，可以直接从您的网站内直接安装各种追加资源。特别是对于那些正在制作大量插件和主题的人，或者那些想创建插件分发网站的人来说，这是一个非常有用的功能。
...
在安装时，为了防止 Misskey Web 接收到被意外篡改，分发站点和 Misskey 都会计算哈希值，并在 Misskey 中进行对照。
...
- `{API_URL}`: 替换为资源分发 API（后面会讲到）的 URL。不能使用相对路径
- `{HASH}`: 将其替换为要分发的资源的 SHA-512 哈希。\*\* 资源中的换行符应统一为LF。\*\*
