...
A partir de Misskey v2026.3.0, además de esta detección de clientes basada en HTML, también se admiten metadatos de clientes en formato JSON.Al proporcionar metadatos de cliente en formato JSON, asegúrese de que se pueda acceder a un documento JSON como el siguiente a través de una dirección HTTPS.Para obtener especificaciones detalladas, consulta [IndieAuth (IndieWeb Living Standard, 11 de julio de 2024)](https://indieauth.spec.indieweb.org/).

Ten en cuenta que, aunque este descubrimiento de clientes comparte algunas similitudes en cuanto al formato con el definido en OpenID Connect Client Metadata 1.0, Misskey no admite OAuth a través de OpenID Connect.

```json
{
  "client_id": "https://example.com", // Obligatorio. El ID del cliente debe ser la URL donde se encuentran los metadatos de este cliente.
  "client_name": "My Misskey App", // Opcional. Especifica el nombre de la aplicación que se mostrará al usuario. Si se omite, se utilizará el ID del cliente como nombre.
  "client_uri": "https://example.com", // Obligatorio. Debe utilizarse como prefijo para el client_id.
  "logo_uri": "https://example.com/logo.png", // Opcional. Puedes especificar un logotipo para la aplicación. Se recomienda utilizar un logotipo con una relación de aspecto de 1:1.
  "redirect_uris": [
    "https://example.com/callback" // Obligatorio. Especifica la URL a la que se enviará el código de verificación.。
  ]
}
```
