...
```html
<!-- (Required) The href address is used as the destination for the authorization code. -->
<link rel="redirect_uri" href="/redirect">

<div class="h-app">
  <!-- (Optional / v2025.3.0 or later) You can specify the app logo. A 1:1 aspect ratio is recommended. -->
  <img src="/logo.png" class="u-logo">
  <!-- (Optional) Specify the app name displayed to users. If not provided, the page URL will be used as the name. -->
  <!-- (Optional) The href attribute must link to this page (i.e., the app introduction page URL). -->
...
Starting with Misskey v2026.3.0, in addition to this HTML-based client discovery, client metadata in JSON format is also supported.If you are providing client metadata in JSON format, please ensure that a JSON document like the following is accessible via an HTTPS address.For detailed specifications, please refer to [IndieAuth (IndieWeb Living Standard 11 July 2024)](https://indieauth.spec.indieweb.org/).

Please note that while this client discovery shares a somewhat common format with what is defined in OpenID Connect's Client Metadata 1.0, Misskey does not support OAuth via OpenID Connect, so please keep this in mind.

```json
{
  "client_id": "https://example.com", // Required. The client ID must be the URL of the location where this client metadata is hosted.
  "client_name": "My Misskey App", // Optional. Specifies the app name shown to users. If absent, the client ID will be used as the name.
  "client_uri": "https://example.com", // Required. Must be a prefix of the client_id.
  "logo_uri": "https://example.com/logo.png", // Optional. You can specify the app's logo. A logo with a 1:1 aspect ratio is recommended.
  "redirect_uris": [
    "https://example.com/callback" // Required. Specifies the destination URL to which the authorization code is redirected.
  ]
}
```
