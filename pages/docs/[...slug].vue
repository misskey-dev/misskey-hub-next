<template>
    ...
    >
        ...
        <div class="pt-6 p-0 sm:p-12 lg:p-6 w-full">
            ...
            <div v-if="data?._TYPE_ === 'STEPPED_GUIDE'" v-show="!guideIndexAutoDetected" class="flex items-center justify-center h-[calc(100vh-4rem)]">
                <MkLoading class="mx-auto text-accent-600"></MkLoading>
            </div>
            <div v-if="data?._TYPE_ === 'STEPPED_GUIDE'" v-show="guideIndexAutoDetected" class="grid" :class="$style.steppedGuideRoot">
                <div class="markdown-body w-full lg:col-span-2 mb-6">
                    <h1>{{ data.title }}</h1>
                    <MDC :value="data?.body" />
                    <Warning :label="null" class="py-4">
