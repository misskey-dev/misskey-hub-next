...
<template>
    ...
    >
        <div class="property-information">
            <div v-if="name" class="property-name">{{ name }}</div>
            <div v-if="required" class="required">required</div>
            <div v-if="value?.type" class="property-type">
                <template v-if="value?.items && !['object'].includes(value.items.type)">
                    {{ value.type }}{{ value.items.type }}[]
                    <template v-if="value.items.example">
                        <code class="property-example-value">example: {{ value.items.example }}</code>
                    </template>
                </template>
                <template v-else>
                    {{ value.type }}
            ...
            <div v-if="value?.writeOnly" class="write-only">write-only</div>
            <div v-else-if="value?.readOnly" class="read-only">read-only</div>
            <template
                v-for="rule in rules"
                :key="rule"
            >
                <div v-if="value?.[rule] || value?.items?.[rule]" class="property-rule-badge">
                    <span class="inline-block bg-slate-200 px-1 py-0.5 rounded-full">{{ rule }}</span>
                </div>
            </template>
            <div v-if="value?.nullable" class="property-nullable">
                nullable
            </div>
            <div v-if="value?.example !== undefined" class="property-example">
                <code class="property-example-value">example: {{ value.example }}</code>
            </div>
        </div>
        <!-- Description -->
        <div
            ...
            <MDC :value="description || value?.description" />
        </div>
        <div
            v-else-if="generatePropertyDescription(value)"
            class="property-description">
            <MDC :value="generatePropertyDescription(value) || ''" />
        </div>
        <!-- Enum -->
        <div
            v-if="value?.enum"
...
.property-example-value {
    padding: 12px 12px;
    box-shadow: 0 0 0 1px var(--theme-border-color, var(--default-theme-border-color));
    background: var(--theme-background-2, var(--default-theme-background-2));
    border-radius: var(--theme-radius, var(--default-theme-radius));
    padding: 2px 5px;
    font-family: var(--theme-font-code, var(--default-theme-font-code));
    font-size: var(--default-theme-font-size-5,
