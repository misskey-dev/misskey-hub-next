...
Misskey v2026.3.0 이후에는 이 HTML의 클라이언트 디스커버리 외에  JSON 형식의 클라이언트 메타 데이터도 지원되게 되었습니다.JSON 형식의 클라이언트 메타 데이터를 제공하는 경우에는 아래와 같은 JSON 문서를 HTTPS 주소로 접근할 수 있게 해주십시오.상세한 방법에 대해서는 [IndieAuth (IndieWeb Living Standard 11 July 2024)](https://indieauth.spec.indieweb.org/)를 참조해 주십시오.

더욱이 이 클라이언트 디스커버리는 OpenID Connect의 Client Metadata 1.0에서 정의돼있는 것과 어느 정도 공통된 형식으로 돼있습니다만, Misskey는 OpenID Connect의 OAuth에는 대응하고 있지 않으므로 그 점에는 유의해 주십시오.

```json
{
  "client_id": "https://example.com", // 필수. 클라이언트 ID는 이 클라이언트 메타 데이터가 있는 장소의 URL로 할 필요가 있습니다.
  "client_name": "My Misskey App", // 임의. 사용자에게 보여주는 애플리케이션의 이름을 지정합니다. 없는 경우에는 클라이언트 ID가 이름으로 됩니다.
  "client_uri": "https://example.com", // 필수. client_id의 접두사로 할 필요가 있습니다.
  "logo_uri": "https://example.com/logo.png", // 임의. 애플리케이션의 로고를 지정할 수 있습니다. 화면 비율이 1:1인 로고를 권장합니다.
  "redirect_uris": [
    "https://example.com/callback" // 필수. 인증 코드의 전송 위치가 될 URL을 지정합니다.
  ]
}
```
